...
flags as (
  select
    *,
    (k_savage_active_revenue > 0) as carries_k_savage,
    (mayfield_active_revenue > 0) as carries_mayfield,
    (leisure_land_active_revenue > 0) as carries_leisure_land,
    ((revenue_total > 0 or k_savage_historical_revenue > 0) and not (k_savage_active_revenue > 0)) as k_savage_lapsed
  from base
),
recommendations as (
  select
...
select
  *,
  ...
  (
    select smp.latest_month
    from public.store_monthly_profile smp
    where smp.store_id = categorized.store_id
  ) as latest_month
from categorized;
